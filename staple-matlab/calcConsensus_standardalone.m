function [apparent3M,staple3M,reliability3M, iterF] = calcConsensus_standardalone(structAll, rtstStructs)
...
a=0;
bigMask = rtstStructs{1}.*0;
for iter1=1:size(structAll, 2)
    mask3M = rtstStructs{structAll(iter1)};
    disp(size(mask3M))
    bigMask=bigMask | mask3M;
end

[iV,jV,kV]=find3d_wjcheon(bigMask);
iMin = min(iV);
iMax = max(iV);
jMin = min(jV);
jMax = max(jV);
...
rateMat = logical([]);
for i=1:length(structAll)
    mask3M = rtstStructs{structAll(i)};
...
[stapleV, sen, spec, Sall, iterF] = staple(rateMat,iterlim, single(senstart), single(specstart));
mean_sen = mean(sen);
std_sen = std(sen);
mean_spec = mean(spec);
std_spec = std(spec);
%get volume of an uniformized voxel (unit)
[xUnifV,yUnifV,zUnifV] = getUniformScanXYZVals_standardalone(rtstStructs);
vol = (xUnifV(2)-xUnifV(1)) * (yUnifV(1)-yUnifV(2)) * (zUnifV(2)-zUnifV(1));
vol= vol.*0.001; % mm3 to cm3 (210830) wjchoen
numBins = 20;
...
disp('-------------------------------------------')
aa= reshape(stapleV,length(iMin:iMax),length(jMin:jMax),length(kMin:kMax));
% staple3M = zeros(getUniformScanSize(planC{indexS.scan}(scanNum)),'single');
staple3M = zeros(size(rtstStructs{1}),'single');
...
reliability3M = zeros(size(rtstStructs{1}),'single');
...
apparent3M  = zeros(size(rtstStructs{1}),'single');
